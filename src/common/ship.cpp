...
Ship::Ship(Mesh::Type type, ShipStats::shared_ptr sstats, float fire_radius):
	...
	if(ctrl != nullptr)
	{
		...
		if(ctrl->shot_countdown <= 0.0f)
		{
			if(ctrl->shot && ctrl->heat < stats->max_heat)
			{
				...
				proj->update(dt, adder);
				adder.add(std::move(proj));
			}
			else
...
std::shared_ptr<Ship> Ship::make_shared_ship(Mesh::Type type, ShipStats::shared_ptr sstats)
{
	if(type == Mesh::Type::DESTROYER) {
