...
RUN apt-get update && \
    apt-get -y install build-essential g++ clang g++-mingw-w64-x86-64 \
    cmake ninja-build libgl1-mesa-dev libglew1.5-dev libsdl2-dev \
    libboost-thread-dev libboost-system-dev libopusfile-dev libopenal-dev && \
    apt-get -y clean

ADD windows-deps.tar.gz /opt
