...
if test $have_gl = yes ; then
  AC_SEARCH_LIBS([glEnable], [GL opengl32], , have_gl=no)
fi

if test $have_gl = yes ; then
  ifelse([$1], , :, [$1])
  LIBGL_LIBS="$ac_cv_search_glEnable"
  LIBGL_CFLAGS=""
else
  ifelse([$2], , :, [$2])
  LIBGL_LIBS=""
...
if test $have_glew = yes ; then
  AC_SEARCH_LIBS([glewInit], [GLEW glew32], , have_glew=no)
fi

if test $have_glew = yes ; then
  ifelse([$1], , :, [$1])
  LIBGLEW_LIBS="$ac_cv_search_glewInit"
  LIBGLEW_CFLAGS=""
else
  ifelse([$2], , :, [$2])
  LIBGLEW_LIBS=""
